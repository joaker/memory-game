...
import { Board } from 'routes/Board/components/Board';
...
describe('(Component) Board', () => {
    ...
    it('renders with an <h2> that includes Board label.', () => {
        expect(_wrapper.find('h2').text()).to.match(/Board:/);
    });

    it('renders {props.board} at the end of the sample board <h2>.', () => {
        expect(_wrapper.find('h2').text()).to.match(/5$/);
        _wrapper.setProps({ board: 8 });
